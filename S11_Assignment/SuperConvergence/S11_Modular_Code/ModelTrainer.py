...
class Train:
  def __init__(self, model, trainloader, dataloader, optimizer, stats, scheduler=None, batch_scheduler=False, L1lambda = 0, LossFunction='CrossEntropyLoss'):
    self.model = model
  ...
  def run(self):
    ...
    with torch.no_grad():
        for data, target in self.dataloader:
            ...
            correct+= pred.eq(target.view_as(pred)).sum().item()
    self.stats.add_batch_val_stats(loss/len(data), correct/len(data), len(data))
...
class ModelTrainer:
  def __init__(self, model, optimizer, train_loader, test_loader, dataloader, statspath, scheduler=None, batch_scheduler=False, L1lambda = 0, LossType='CrossEntropyLoss'):
    self.model = model
    self.scheduler = scheduler
    self.batch_scheduler = batch_scheduler
    self.optimizer = optimizer
    ...
    self.train = Train(model, train_loader, dataloader, optimizer, self.stats, self.scheduler if self.scheduler and self.batch_scheduler else None, self.batch_scheduler, L1lambda, self.LossFunction)
